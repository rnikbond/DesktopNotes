...
Err DesktopNotes::createNoteDB( const QString& uuid, const QString& title, const QString& text ) {
    ...
    QString sql = QString("INSERT INTO %1 VALUES(:uuid, :title, :text)").arg(TABLE_NOTES);
...
Err DesktopNotes::deleteNoteDB( const QString& uuid ) {
    ...
    QString sql = QString("DELETE FROM %1 WHERE %2 = :uuid").arg(TABLE_NOTES, NOTES_FIELD_UUID);
